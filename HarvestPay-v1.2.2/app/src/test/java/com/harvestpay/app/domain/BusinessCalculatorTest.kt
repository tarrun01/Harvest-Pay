...
class BusinessCalculatorTest {
    ...
    @Test
    fun `previous due is cleared before work and overpayment becomes advance`() {
        ...
        val summary = BusinessCalculator.buildUiState(
            listOf(customer),
            emptyList(),
            listOf(work),
            listOf(payment),
        ).customerSummaries.single()

        assertEquals(300.0, summary.previousDue, 0.001)
        assertEquals(800.0, summary.workTotal, 0.001)
        assertEquals(1_100.0, summary.totalBill, 0.001)
        assertEquals(100.0, summary.advance, 0.001)
        assertEquals(800.0, summary.work.single().paid, 0.001)
    }

    @Test
    fun `diesel dashboard totals include each saved entry once`() {
        val diesel = listOf(
            DieselEntryEntity(
                id = 1,
                litres = 12.5,
        ...
        val stats = BusinessCalculator.buildUiState(
            emptyList(),
            emptyList(),
            emptyList(),
            emptyList(),
        ...
        assertEquals(19.75, stats.totalDieselLitres, 0.001)
        assertEquals(1_750.0, stats.totalDieselAmount, 0.001)
    }
}
