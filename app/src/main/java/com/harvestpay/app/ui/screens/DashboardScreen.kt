    ...
    LazyColumn(
        ...
        item {
            Column(Modifier.padding(top = 12.dp)) {
                Text("Namaste", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.primary)
                Text("Today’s business at a glance", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
            }
        }
        item {
            MetricGrid(
        ...
        val pending = uiState.customerSummaries.filter { it.pending > 0.005 }.sortedByDescending { it.pending }
        if (pending.isEmpty()) {
            item { EmptyState("All caught up", "There are no pending customer balances.") }
        } else {
            items(pending.take(4), key = { "pending-${it.customer.id}" }) { summary ->
                PendingCustomerCard(
                    summary = summary,
